-- Revised fix for admin RPC functions (version 2)
...
-- But let's check if a policy for users already exists before creating it
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy
    WHERE polname = 'users_read_own_profile'
...
-- 7. Add a policy for updating their own profile (if it doesn't exist yet)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy
    WHERE polname = 'admin_update_own_profile'
    AND polrelid = 'public.profiles'::regclass
  ) THEN
    EXECUTE 'CREATE POLICY admin_update_own_profile ON public.profiles
              FOR UPDATE
              USING (auth.uid() = id)';
  END IF;
END
