...
export function toIsraelTime(date: Date): Date {
  ...
  const dateStr = formatInTimeZone(date, ISRAEL_TIMEZONE, "yyyy-MM-dd'T'HH:mm:ss.SSS");
...
export function formatTimeRangeInIsrael(start: Date, end: Date): string {
  return `${formatInTimeZone(start, ISRAEL_TIMEZONE, 'h:mm aa')} - ${formatInTimeZone(end, ISRAEL_TIMEZONE, 'h:mm aa')}`;
}
...
export function createIsraelDate(
  ...
  const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}T` +
