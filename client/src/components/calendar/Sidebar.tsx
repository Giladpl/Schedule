...
import { RefreshCcw, Phone, Video, Users } from "lucide-react";
...
export default function Sidebar({
  ...
  const { mutate: refreshRules, isPending: isRefreshing } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/client-meeting-types'] });
  ...
  const getAvailableMeetingTypes = (): string[] => {
    ...
    const selectedClient = clientData.clients.find(client => client.type === clientType);
  ...
  return (
    <div className="hidden md:block w-64 border-r border-[#dadce0] p-4 py-8">
        ...
        {isLoadingClientData ? (
          ...
          >
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="all">
                {getClientTypeLabel("all")}
              </Label>
            </div>
            {clientData?.clients && (() => {
              // Use the client data from our new structure
              return clientData.clients.map(client => (
                <div key={client.type} className="flex items-center space-x-2">
                  ...
                  <Label htmlFor={client.type}>
                    {getClientTypeLabel(client.type) || client.type}
                  </Label>
                </div>
              ));
            })()
      ...
      <div className="mb-8">
        <h2 className="text-sm font-medium text-[#5f6368] mb-4">MEETING TYPE</h2>
        {isLoadingClientData ? (
          <div className="space-y-2">
            <Skeleton className="h-6 w-full" />
            <Skeleton className="h-6 w-full" />
          ...
          >
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="all" id="meeting-all" className="text-[#1a73e8]" />
      ...
      <div className="mb-8">
        ...
        <div className="space-y-3">
          ...
          <h3 className="text-xs font-medium text-[#5f6368] mt-3">Client Types:</h3>
          <div className="flex items-center">
      ...
      <div>
        <h2 className="text-sm font-medium text-[#5f6368] mb-4">ADMIN CONTROLS</h2>
        <Button 
          variant="outline" 
          size="sm" 
